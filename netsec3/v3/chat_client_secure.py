...
try:
    ...
    from . import config
    from . import ns_handshake
    from .ns_handshake import (
        request_session_key,
        handle_ns_resp,
        handle_ns_ticket,
        complete_ns_auth,
    ...
    import ns_handshake  # type: ignore
    from ns_handshake import (
        request_session_key,
        handle_ns_resp,
        handle_ns_ticket,
...
ns_handshake.set_state(handshake_events, session_keys, client_username, channel_sk)
...
def handle_encrypted_payload(payload: dict) -> None:
    ...
    if msg_type == "AUTH_RESPONSE":
        ...
        if user:
            ...
            console.print(f"<Server> {user} signed out.", style="server")
